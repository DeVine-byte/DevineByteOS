...
public class WorkflowEngine {
    ...
    public WorkflowEngine(WorkflowInstanceRepository repo, WorkflowExecutor executor, KPIEngine kpiEngine) {
        this.repo = repo;
    ...
    public void handleEvent(TenantContext ctx, UUID instanceId, DomainEvent event) {
        ...
        if (instance.tenant() != null && !ctx.tenantId().equalsIgnoreCase(instance.tenant().tenantId())) {
            System.err.printf(
                    "[SECURITY GUARD] Fatal Ingress Violation: Mismatched tenant context detected. Access denied to instance %s%n",
                    instanceId);
            throw new IllegalArgumentException("Security Restriction Violation: Mismatched routing scope parameters.");
        }

        WorkflowDefinition def = definitions.get(instance.workflowName());
        if (def == null) return;

        WorkflowInstance advanced = executor.handleEvent(ctx, def, instance, event);
        repo.save(advanced);
    ...
    public Object start(TenantContext ctx, String command, JsonNode body, String commandOrQuery) {
        ...
        String normalizedCmd = command != null ? command.trim().toLowerCase() : "";
        ...
        if (normalizedCmd.contains("fetchdashboardmetrics")
            ...
            if (kpiEngine == null) {
                throw new IllegalStateException("Dashboard KPI engine dependency is completely unassigned.");
            }
        ...
        Map<String, Object> runtimeContext = input != null ? new HashMap<>(input) : new HashMap<>();
        ...
        if (lowerEntity.contains("appointment") || lowerEntity.contains("invoice")) {
            moduleId = "sales";
        } else if (lowerEntity.contains("stock") || lowerEntity.contains("prescription")) {
            moduleId = "inventory";
        } else if (lowerEntity.contains("payment") || lowerEntity.contains("insurance")) {
            moduleId = "finance";
        }
        ...
        if (command.startsWith("Handle") && command.endsWith("POST")) {
            try {
                ...
                if (naturalUniqueKey != null) {
                    String uniqueValue = String.valueOf(dataPayload.get(naturalUniqueKey));
                    ...
                    if (!uniqueValue.trim().isEmpty()) {
                        ...
                        Field dsField = entityRepo.getClass().getDeclaredField("ds");
                        dsField.setAccessible(true);
                        DataSource ds = (DataSource) dsField.get(entityRepo);

                        String table = (moduleId + "_" + entityName).toLowerCase();
                        String checkSql = "SELECT payload FROM " + table;
                            ...
                            while (rs.next()) {
                                ...
                                if (record.containsKey(naturalUniqueKey)
                                        && uniqueValue.equalsIgnoreCase(
                                                String.valueOf(record.get(naturalUniqueKey)))) {
                                    System.err.println(
                                            "[CORE REJECTION] Duplicate identity conflict found for asset: "
                                                    + uniqueValue);
                                    throw new IllegalStateException(
                                            "409 Conflict: An operational entity matching unique constraints "
                                                    + "already exists inside the system runtime.");
                                }
                            }
                        }
                    }
                ...
                System.out.println(
                        "[ENGINE DEBUG] Successfully wrote full record to DB with ID: " + persistentId);

                if (kpiEngine != null) {
                    kpiEngine.recordMetric(tenantId, entityName + "Created_COUNT", 1.0);

                    if (dataPayload.containsKey("status") && dataPayload.get("status") != null) {
                        String statusVal = String.valueOf(dataPayload.get("status")).trim();
                        kpiEngine.recordMetric(tenantId, entityName + "Updated_COUNT", 1.0);
                        kpiEngine.recordMetric(tenantId, entityName + "_" + statusVal + "_COUNT", 1.0);
                    }
                }
                ...
                try {
                    ...
                    boolean isStatusUpdate = dataPayload.containsKey("id")
                            || (dataPayload.containsKey("status")
                            && !"SCHEDULED".equalsIgnoreCase(eventPayload.get("status").asText()));

                    String derivedEventType = isStatusUpdate
                            ? entityName + "Updated"
                            : entityName + "Created";

                    System.out.println(
                            "[NOTIFICATION DESCRIPTOR] Inferred Event Type Channel => "
                                    + derivedEventType);
                    ...
                    NotificationCenterHandler directAlertHandler =
                            new NotificationCenterHandler(domainEvent.type());
                    ...
                    try {
                        ...
                        policyEngine.evaluateEvent(
                                ...
                                domainEvent,
                                this.kpiEngine != null
                                        ? this.kpiEngine.getTenantMetricsGrid(tenantId)
                                        : null);
                    } catch (Exception ignored) {
                    }
                } catch (Exception e) {
                    System.err.println(
                            "[EVENTSTREAM ERROR] Direct bypass notification dispatch failed: "
                                    + e.getMessage());
                }
        ...
        if (command.startsWith("Handle") && command.endsWith("PUT")) {
            try {
                EntityRepository entityRepo = RepositoryFactory.get(tenantId, moduleId, entityName);
                ...
                if (targetId == null || targetId.trim().isEmpty()) {
                    throw new IllegalArgumentException(
                            "400 Bad Request: Missing entity record tracking identifier 'id' for PUT updates.");
                }

                System.out.println(
                        "[ENGINE UPDATE] Merging incoming payload modifications onto record ID: "
                                + targetId);

                Map<String, Object> existingRecord = entityRepo.findById(targetId);

                if (existingRecord == null) {
                    throw new NoSuchElementException(
                            "404 Not Found: Cannot perform update. Entity matching ID '"
                                    + targetId
                                    + "' does not exist.");
                }

                existingRecord.putAll(dataPayload);
                existingRecord.put("id", targetId);

                System.out.println(
                        "[ENGINE UPDATE] Persisting merged record layout for ID: " + targetId);

                String persistentId = entityRepo.upsert(existingRecord);
                ...
                try {
                    ...
                    for (Map.Entry<String, Object> entry : existingRecord.entrySet()) {
                        if (entry.getValue() != null) {
                            eventPayload.put(entry.getKey(), entry.getValue().toString());
                        }
                    }
                    ...
                    System.err.println(
                            "[EVENTSTREAM ERROR] Direct bypass update notification dispatch failed: "
                                    + e.getMessage());
                }

                existingRecord.put("status", "SUCCESS");
        ...
        if (command.startsWith("Handle") && command.endsWith("GET")) {
            try {
                ...
                System.out.println(
                        "[ENGINE QUERY] Fetching full database record for ID: " + targetId);
        ...
        if (def == null) {
            throw new IllegalArgumentException(
                    "No workflow definition registered for command: " + command);
        }
        ...
        return executor.start(ctx, def, runtimeContext);
    }
